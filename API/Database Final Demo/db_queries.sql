...
SELECT 
    ...
    ymh01.ymh01f05 AS 'Return Date'       -- Return Date
FROM 
    ymh01
JOIN 
...
WHERE 
    ymh01f05 IS NULL;  -- Books that have not been returned yet
...
FROM 
    ymb01
WHERE 
    ymb01.ymb01f05 = 'Science Fiction'     -- Condition for Science Fiction books
...
FROM 
    ymb01
WHERE 
    ymb01.ymb01f04 = 2020                   -- Condition for books published in 2020
...
FROM 
    ymb01
JOIN 
    ymh01 ON ymb01.ymb01f01 = ymh01.ymh01f02  -- Join with Book History table
WHERE 
    ymh01.ymh01f04 IS NOT NULL;                -- Condition for borrowed books (Issue Date is not NULL)
    
-- Data sorting demo

-- Sort books by title in ascending order (A-Z)
SELECT 
    ymb01.ymb01f01 AS 'Book ID',
    ymb01.ymb01f02 AS 'Book Title',
    ymb01.ymb01f03 AS 'Author',
    ymb01.ymb01f04 AS 'Published Year',
    ymb01.ymb01f05 AS 'Category'
FROM 
    ymb01
ORDER BY 
    ymb01.ymb01f02 ASC;  -- Sorts by Book Title in ascending order
...
FROM 
    ymb01
ORDER BY 
    ymb01.ymb01f04 DESC;  -- Sorts by Published Year in descending order
...
SELECT 
    ymb01.ymb01f05 AS 'Category',           -- Category
    COUNT(ymb01.ymb01f01) AS 'Total Books'  -- Count of books in each category
FROM 
    ymb01
GROUP BY 
    ymb01.ymb01f05;                         -- Group by category

-- Find the average published year of books in the library
SELECT 
    AVG(ymb01.ymb01f04) AS 'Average Published Year'  -- Average of Published Year
FROM 
    ymb01;

-- Find the maximum and minimum published year of books in the library
SELECT 
    MAX(ymb01.ymb01f04) AS 'Most Recent Published Year',  -- Maximum Published Year
    MIN(ymb01.ymb01f04) AS 'Oldest Published Year'         -- Minimum Published Year
FROM 
    ymb01;
...
FROM 
    ymb01
WHERE 
    ymb01.ymb01f01 IN (                     -- Subquery to find Book IDs borrowed by member ID = 1
        SELECT ymh01.ymh01f02                 -- Book ID from the Book History table
        FROM ymh01
        WHERE ymh01.ymh01f03 = 1              -- Member ID = 1
        AND ymh01.ymh01f05 IS NULL           -- Books that have not been returned yet
    );
...
FROM 
    ymb01
ORDER BY 
    ymb01.ymb01f04 DESC                      -- Order books by published year in descending order
